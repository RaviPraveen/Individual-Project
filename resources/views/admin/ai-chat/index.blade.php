<x-admin-layout>
    ...
    <p class="text-muted small">{{ __('Ask about product availability, pricing, stock levels, sales summaries, or reorder recommendations.') }}</p>

    <div class="card mb-3">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.ai-chat.ask') }}">
                @csrf
                <div class="mb-3">
                    <textarea name="message" class="form-control" rows="2" placeholder="{{ __('e.g. Which products need reordering soon?') }}" required>{{ old('message') }}</textarea>
